...
class GraphManager:
    ...
    def expand_node(self, iteration_num: int) -> Optional[NodeInfo]:
        ...
        if expandable_nodes:
            parent_node = max(expandable_nodes, key=lambda x: self.storage.get_iteration(x.iteration).get_evaluation().similarity)
    ...
    def print_tree(self, iteration_marker: int, best_marker: bool) -> str:
        ...
        if best_marker:
            best_node = max(self.storage.get_iterations(), key=lambda x: x.get_evaluation().similarity).node
        else:
            best_node = None
    ...
    def _build_tree_string(self, node: NodeInfo, iteration_marker: int, best_node: Optional[NodeInfo],
        ...
        fitness = iteration.get_evaluation().similarity if iteration else "N/A"
