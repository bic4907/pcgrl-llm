...
from envs.probs.dungeon4 import Dungeon3Tiles
from envs.probs.problem import draw_solutions
from envs.solution import get_solution
from envs.solution2 import get_solution2
...
def render_level(array: np.array, tile_size: int = 16,
                 return_numpy: bool = False,
                 is_scenario2: bool = False,
                 scenario2_imp: int = Dungeon3Tiles.KEY,
                 draw_solution: bool = True):
    """
    ...
    if not is_scenario2:
        ...
        tiles = {
            ...
            4: Image.open(join(image_dir, 'bat.png')),
            5: Image.open(join(image_dir, 'scorpion.png')),
            6: Image.open(join(image_dir, 'door.png')),
            7: Image.open(join(image_dir, 'key.png')),
            8: Image.open(join(image_dir, 'treasure.png')).convert("RGBA"),
        }
    ...
    if draw_solution:
        if not is_scenario2:
            ...
            solutions = get_solution2(level, imp_tile=scenario2_imp)
            img = draw_solutions(lvl_img=img, solutions=solutions, tile_size=tile_size)
    if return_numpy:
        return np.array(img)
    else:
